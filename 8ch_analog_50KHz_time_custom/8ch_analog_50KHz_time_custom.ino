...
int16_t raw1, raw2, raw3, raw4, raw5, raw6, raw7, raw8, raw9, raw10, diff;
unsigned long lastMicros_1, lastMicros_2, timer1, timer2;
uint32_t timestamp_micros = 0;
uint32_t custom_SYNC_WORD = 0x754D754D;
...
void loop() {
  ...
  if (micros() >= lastMicros_1 + 20)  // micros() >= lastMicros + DESIRED_LOOP_DELAY_uSec
  {
    ...
    timestamp_micros = 0xBC614E;  //12345678 in decimal
    ...
    Serial.write((uint8_t *)&(custom_SYNC_WORD), sizeof(custom_SYNC_WORD));
    Serial.write((uint8_t *)&(timestamp_micros), sizeof(timestamp_micros));
